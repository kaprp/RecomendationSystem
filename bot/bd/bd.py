...
def add_user(user:User):
    try:
       ...
       cursor.execute("insert into Настройки (ID, use_link, markets_link, viewed_marketplaces) values (?,?,?,?)", user_id, settings.use_link, settings.markets_link, settings.viewed_marketplaces)
       connect.commit()
...
def get_settings(id:int):
    try:
       ...
       cursor.execute("select use_link, markets_link, viewed_marketplaces from Настройки where ID = ? ", user_id)
       row = cursor.fetchone()
       if row == None: #Если вдруг нет информации о настройках, то задаём стандартные
           settings = Settings()
        ...
        settings = Settings(row.use_link, row.markets_link, row.viewed_marketplaces)
...
def change_settings(id:int, settings:Settings):
    try:
        connect = pyodbc.connect(config_connection)
        cursor = connect.cursor() 
        ...
        cursor.execute("update Настройки set use_link = (?), markets_link = (?), viewed_marketplaces = (?) where ID = ? ",settings.use_link, settings.markets_link, settings.viewed_marketplaces, user_id)
        connect.commit()
        ...
        logging.error("Can't connection to DB while try change settings", exc_info=True)
        print("Error in connection while try change settings")
